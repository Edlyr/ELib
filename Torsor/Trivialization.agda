...
module Trivializations (A : AbGroup {ℓ}) where
  ...
  principalLink = makeLink-pnt (groupequiv eq isHom)
    where open GroupEquiv (finalLemmaAb (GRP A) (AbGroup.comm A))
  ...
  module _ (G : B² A {ℓ}) where
    ...
    trivializeGerbe : G.Carrier → B²Equiv A G torsors
    trivializeGerbe x₀ = b²equiv (triv x₀) lemma where
      ...
      abstract
        ...
        lemma = congLink-carac _ _ _ x₀ ∙ groupHomEq (
          ...
          (λ a → transport (cong Torsor.Carrier (p' (invEq (G.eq x₀) a) ∙ p)) 0g)
            ≡⟨ funExt subLemma3 ⟩
          (λ a → a) ∎) where
          open GroupEquiv (finalLemmaAb (GRP A) (AbGroup.comm A))
          p : T₀ ≡ TA
    ...
    deTrivialize : B²Equiv A G torsors → G.Carrier
    deTrivialize eq = invEq (B²Equiv.eq eq) TA

    retr : retract trivializeGerbe deTrivialize
    retr x₀ =
      invEq eq TA          ≡⟨ cong (invEq eq) lemma ⟩
      invEq eq (fst eq x₀) ≡⟨ secEq eq x₀ ⟩
      x₀ ∎ where
      eq = B²Equiv.eq (trivializeGerbe x₀)
      lemma : TA ≡ fst eq x₀
      lemma = uaTorsor (trivialize (triv x₀ x₀) refl)

    sec : section trivializeGerbe deTrivialize
    sec eq = b²EquivEq _ _ (cong fst (deloopUnique _ _ _ _ _ fun2 fun1)) where
      x₀ : ⟨ G.grb ⟩
      x₀ = invEq (B²Equiv.eq eq) TA

      fun1 : deloopType G.lnk PG.lnk (grouphom (λ x → x) (λ _ _ → refl)) x₀ TA
      fun1 = B²Equiv.fun eq , sym (retEq (B²Equiv.eq eq) TA) , B²Equiv.p eq

      fun2 : deloopType G.lnk PG.lnk (grouphom (λ x → x) (λ _ _ → refl)) x₀ TA
      fun2 = triv (deTrivialize eq) , uaTorsor (trivialize (triv x₀ x₀) refl) , B²Equiv.p (trivializeGerbe (deTrivialize eq))

    GerbeTrivialization : G.Carrier ≃ (B²Equiv A G torsors)
    GerbeTrivialization = isoToEquiv (iso trivializeGerbe deTrivialize sec retr)
